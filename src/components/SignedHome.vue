<template>
    <div style="overflow: hidden">
        ...
        <div class="uk-margin-large-top uk-margin-right uk-margin-medium-bottom">
            ...
            <center class="uk-text-large">{{ $t('signed_home.words') }}</center>
            <vue-word-cloud
            id="wcloud"
            style="
                width: 75vw;
                ...
                <div class="uk-grid-medium uk-flex-middle" uk-grid>
                    <div class="uk-width-expand">
                        <h4 class="uk-comment-title uk-margin-remove"><a class="uk-link-reset" href="#">
                            {{ $t('signed_home.voiced') }}{{ v.voice == null ? 'Deleted voice' : v.voice }}
                        </a></h4>
...
export default {
    ...
    data() {
        return {
            langCodes: {
                'uk': this.$t('signed_home.uk'),
                'en': this.$t('signed_home.en')
            },
            queries: [],
            word_list: []
        };
    ...
    mounted: async function () {
        ...
        _this.$http.get('http://34.118.9.73:8080/api/organization/')
        .then(rsp => {
            this.$http
            .get('http://34.118.9.73:8080/api/query/?organization=' + rsp.data.public_id)
            .then(resp => {
                ...
                var trace11 = {
                    ...
                    name: _this.$t("signed_home.sum_tokens"),
                    type: 'scatter'
                };
                var trace12 = {
                    x: Object.keys(queriesByDate),
                    y: Object.values(queriesByDate).map(v => (v.map(x => x.text.length).reduce((a, b) => a + b, 0) / v.length).toFixed(2)),
                    name: _this.$t('signed_home.avg_tokens'),
                    type: 'scatter'
                };
                var trace13 = {
                    x: Object.keys(queriesByDate),
                    y: Object.values(queriesByDate).map(v => v.length),
                    name: _this.$t('signed_home.num_q'),
                    type: 'scatter'
                };

                plot_timeseries([trace11, trace12, trace13], _this.$t('signed_home.qd'), 'date_tokens');

                var trace21 = {
                    x: Object.keys(queriesByDate),
                    y: Object.values(queriesByDate).map(v => v.map(x => x.time_processed).reduce((a, b) => a + b, 0)),
                    name: _this.$t('signed_home.gen_time'),
                    type: 'scatter'
                };
                var trace22 = {
                    x: Object.keys(queriesByDate),
                    y: Object.values(queriesByDate).map(v => (v.map(x => x.time_processed).reduce((a, b) => a + b, 0) / v.length).toFixed(2)),
                    name: _this.$t('signed_home.avg_time'),
                    type: 'scatter'
                };

                plot_timeseries([trace21, trace22, trace13], _this.$t('signed_home.qtd'), 'query_time');
